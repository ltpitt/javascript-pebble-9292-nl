...
- Always verify project structure first:
  ```bash
  ls -la
  # Expected: LICENSE, README.md, appinfo.json, resources/, pebble-app/, TASKS.md
  ```
- Validate appinfo.json: `cat appinfo.json | python3 -m json.tool`
- Validate JavaScript syntax: `node -c pebble-app/js/app.js`

### JavaScript Code Quality
- Install linting tools: `npm install --save-dev jshint eslint` -- takes 10-15 seconds
- Run JavaScript validation: `npx jshint pebble-app/js/app.js`
...
- **ALWAYS run before any changes**: `node -c pebble-app/js/app.js`
- **Check code quality**: `npx jshint pebble-app/js/app.js` 
...
2. **ALWAYS validate syntax first**: `node -c pebble-app/js/app.js`
3. **Run linting**: `npx jshint pebble-app/js/app.js`
...
- **API endpoints**: OV API integration in `pebble-app/js/app.js`
...
- **View source code**: `cat pebble-app/js/app.js` -- instant
...
- **`pebble-app/js/app.js`**: Main application logic using Pebble.js APIs (ui, ajax, settings)
- **`pebble-app/js/main.js`**: Pebble.js framework loader
- **`pebble-app/main.c`**: Native C host for Pebble.js runtime
...
- **Solution**: Validate JavaScript with `node -c pebble-app/js/app.js`
...
- **Validate**: Run `npx jshint pebble-app/js/app.js` for code quality issues
- **Test**: Use console.log() statements for debugging

### CI/CD Considerations
Manual validation workflow for each change:
1. **Always run**: `node -c pebble-app/js/app.js` for syntax validation (1-2 seconds)
2. **Always run**: `npx jshint pebble-app/js/app.js` for code quality (1-2 seconds)
